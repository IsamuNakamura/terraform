...
resource "aws_iam_role" "codebuild_migration_undo" {
  for_each = { for service_name, environment_variable in var.environment_variables : service_name => environment_variable }

  name               = "${var.label}-${var.tier}-codebuild-migration-undo-${each.key}"
  description        = "codebuild service role for migration undo"
...
resource "aws_iam_role_policy" "codebuild_migration_undo_role_policy" {
  for_each = { for service_name, iam_role in aws_iam_role.codebuild_migration_undo : service_name => iam_role }

  name   = aws_iam_role.codebuild_migration_undo[each.key].name
  role   = aws_iam_role.codebuild_migration_undo[each.key].id
  policy = data.aws_iam_policy_document.codebuild_migration_undo_policy[each.key].json
}

data "aws_iam_policy_document" "codebuild_migration_undo_policy" {
  for_each = { for service_name, environment_variable in var.environment_variables : service_name => environment_variable }
  ...
  statement {
    ...
    resources = [
      ...
      "*"
      # aws_codebuild_project.migration_undo[each.key].arn,
    ]
  }
...
resource "aws_iam_instance_profile" "codebuild_migration_undo" {
  for_each = { for service_name, iam_role in aws_iam_role.codebuild_migration_undo : service_name => iam_role }

  name = "${var.label}-${var.tier}-codebuild-migration-undo-${each.key}"
  role = aws_iam_role.codebuild_migration_undo[each.key].name
}
