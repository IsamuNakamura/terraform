resource "aws_codebuild_project" "migration_up" {
  ...
  vpc_config {
    ...
    security_group_ids = [aws_security_group.migration_up.id]
  }
  ...
  source {
    ...
    git_clone_depth = 1
    buildspec       = file("${path.cwd}/buildspec/migration/up.yml")
  }
